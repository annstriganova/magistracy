...
* [Исправление неисправностей](#исправление-неисправностей)
  * [`error: no such device`/`BAD SYSTEM #0`](#error-no-such-device-bad-system-#0)
  * [Смена пароля `root`](#cмена-пароля-root)
  * [Бесконечный рестарт системы или зависание](#бесконечный-рестарт-системы)
  * [`Failed to start Network Time Synchronization\Login Service`](#failed-to-start-network-time-synchronization-login-service)
...
**Причина**: некорректные параметры в `/etc/passwd` и `/etc/shadow` для `root`.
